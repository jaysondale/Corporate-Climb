...
public class GameManager : MonoBehaviour
{
    ...
    // Influence related parameters
    public int PromotionInfluence;

    public int WinGameInfluence;

    public int FiredInfluence;

    public int InternStartingInfluence;

    public int InternComplimentSuccessRate;

    public int InternOfferSuccessRate;

    public int InternInsultSuccessRate;

    public int ManagerStartingInfluence;

    public int ManagerComplimentSuccessRate;

    public int ManagerBribeSuccessRate;

    public int ManagerInsultSuccessRate;

    public int WinGameCash = 2000;

    public int PromotionCash = 2000;
    ...
    // Getter and setter for leve: related to the job position variable
    private int _level
    {
        get
        {
    ...
    public int partnerInfluencePenalty = 10;
    ...
    // Stores the name and influence of managers
    private SortedDictionary<string, int> _managers;
    ...
    private void Awake()
    {
        if (Instance == null)
        {
            ...
            _managers = new SortedDictionary<string, int>();

            // Interns
            _bots["Avery"] = InternStartingInfluence;

            _bots["Paul"] = InternStartingInfluence;

            _bots["James"] = InternStartingInfluence;

            // Managers (Appear on level 2)
            _managers["Bill"] = ManagerStartingInfluence;

            _managers["Dave"] = ManagerStartingInfluence;

            _managers["Sam"] = ManagerStartingInfluence;
    ...
    public int GetManagerInfluence(string managerName)
    {
        return _managers[managerName];
    }

    public SortedDictionary<string, int> GetManagers()
    {
        return _managers;
    }
    ...
    public string GetBotStats()
    {
        ...
        if (_level == 2)
        {
            foreach (KeyValuePair<string, int> manager in _managers)
            {
                retString += "Manager " + manager.Key + ": " + manager.Value + " influence\n";
            }
        }
        return retString;
    }
    ...
    public void IncrementManagerInfluence(string name, int increment)
    {
        _managers[name] += increment;
    }
    ...
    public void DestroyManager(string name)
    {
    ...
    {
        if (_influence >= PromotionInfluence && _cash >= PromotionCash && _level == 1)
        {
            _influence = 0;
        ...
        // Win functionality
        if (_influence >= WinGameInfluence && _cash >= WinGameCash && _level == 2)
        {
            GlobalAlert("Congradulations! Your influence is unmatched. You are now the CEO of quantbit!");
            Invoke("ResetGame", 4);
        }
    }

    // Checks to see if any bots have reach promotional influence. If any one of them does, you lose the game.
    void CheckBots()
    {
        foreach (string bot in _bots.Keys)
        {
            // Use PromotionInfluence on level 1, WinGameInfluence on level 2
            if (_bots[bot] >= (_level == 1 ? PromotionInfluence : WinGameInfluence))
            {
                ...
                GameObject botToRemove = null;
                // Remove bot
                foreach (GameObject botObject in Bots)
                {
                    if (botObject.GetComponent<BotController>().GetCharacterName() == bot)
                    {
                        botToRemove = botObject;
                        break;
                    }
                }


                // Double check to ensure a bot was found
                if (botToRemove != null)
                {
        ...
        // Check to see if any managers got promoted
        if (_level == 2)
        {
            foreach (string manager in _managers.Keys)
            {
                if (_managers[manager] >= WinGameInfluence)
                {
                    GlobalAlert("Oh no! Looks like " + manager + " got promoted instead of you. Next time make sure none of your colleagues get too much influence!");
                    _influence = 0;

                    GameObject botToRemove = null;
                    // Remove bot
                    foreach (GameObject managerObject in Managers)
                    {
                        if (managerObject.GetComponent<BotController>().GetCharacterName() == manager)
                        {
                            botToRemove = managerObject;
                            break;
                        }
                    }
    ...
    {
        if (_influence >= FiredInfluence && influenceChange < 0 && _influence + influenceChange >= FiredInfluence && _influence + influenceChange < 0)
        {
    ...
    public void PartnerInfluenceChange(string managerName, float multiplier)
    {
        foreach (KeyValuePair<string, int> manager in _managers)
        {
            if (manager.Key != managerName)
            {
                _managers[manager.Key] = manager.Value + Mathf.RoundToInt(partnerInfluencePenalty * multiplier);
            }
        }
    }


    // Toggles the character (hans vs. kira) -> used in character customization
    ...
    public void GoToVideoGame(string ownerName)
    {
        _currentSceneName = _sceneNames[6];
        _isEarning = false;
        UpdateScene();
        _managerOwner = ownerName;
    }
