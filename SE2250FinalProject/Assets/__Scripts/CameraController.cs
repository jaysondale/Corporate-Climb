...
public class CameraController : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        foreach (KeyValuePair<string, int> bot in _bots)
        {
            ...
            botController.SetOfferSuccess(_gameManager.InternOfferSuccessRate);
            botController.SetComplimentSuccess(_gameManager.InternComplimentSuccessRate);
            botController.SetInsultSuccess(_gameManager.InternInsultSuccessRate);
        ...
        if (_gameManager.GetLevel() == 2)
        {
            ...
            foreach (KeyValuePair<string, int> manager in _gameManager.GetManagers())
            {
                ...
                botController.SetBribeSuccess(_gameManager.ManagerBribeSuccessRate);
                botController.SetComplimentSuccess(_gameManager.ManagerComplimentSuccessRate);
                botController.SetInsultSuccess(_gameManager.ManagerInsultSuccessRate);
    ...
    void Update()
    {
        ...
        if (_gameManager.GetCodeReviewActive())
        {
            ...
            codeReviewTimerText.text = "Work Due In: " + minutes + ":" + seconds.ToString("D2");
        } else
        {
    ...
    void UpdateInstructionsText()
    {
        ...
        {   // this will return text that outlines how far the player is from a promotion in influence and cash, where appropriate (if you already
            // have enough cash, it won't include a negative value in the text)
            string text = "Current Position: " + _gameManager.GetPosition() + "\n";

            int influenceThreshold = _gameManager.GetLevel() == 1 ? _gameManager.PromotionInfluence : _gameManager.WinGameInfluence;
            if (_gameManager.GetInfluence() < influenceThreshold)
            {
                text = text + ( influenceThreshold - _gameManager.GetInfluence()) + " influence ";
            }

            int cashThreshold = _gameManager.GetLevel() == 1 ? _gameManager.PromotionCash : _gameManager.WinGameCash;
            if (_gameManager.GetCash() < cashThreshold)
            {
                text = text + "and $" + (cashThreshold - _gameManager.GetCash()) + " ";
            }

            text = text + "to a promotion!";
